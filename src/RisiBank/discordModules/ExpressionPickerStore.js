...
class ExpressionPickerStore {
    ...
    constructor() {
        const ExpressionPickerModule = getModule((m) =>
            Object.keys(m).some(
                (key) =>
                    typeof m[key] === "function" && m[key].toString().includes("isSearchSuggestion")
            )
        );

        Object.values(ExpressionPickerModule).forEach((fn) => {
            if (fn.getState) this.useExpressionPickerStore = fn;
            else if (fn.toString().includes("===")) this.toggleExpressionPicker = fn;
...
export { EPS as ExpressionPickerStore };
